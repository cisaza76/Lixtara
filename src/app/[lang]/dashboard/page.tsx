...
// Whole days a listing has been live (request-scoped; server component).
...
export default async function DashboardPage({
    ...
    await Promise.all([
      ids.length > 0
        ? supabase
            ...
            .select("property_id,url,is_primary,display_order,is_staged")
            .in("property_id", ids)
      ...
      ids.length > 0
        ? supabase
            .from("tour_jobs")
            .select("property_id,tour_kind,status")
            .in("property_id", ids)
  ...
  // Staged-photo count per listing (is_staged photos generated by AI staging).
  const stagedCountByProperty = new Map<string, number>();
  for (const ph of (photoRows ?? []) as PhotoRow[]) {
    if (ph.is_staged)
      stagedCountByProperty.set(
        ph.property_id,
        (stagedCountByProperty.get(ph.property_id) ?? 0) + 1,
      );
  }
  // Living Listing video status per listing: best of any video tour_jobs row
  // (ready > processing/queued > failed > none).
  const VIDEO_RANK: Record<string, number> = {
    ready: 4,
    processing: 3,
    queued: 3,
    uploading: 3,
  ...
  const videoStatusByProperty = new Map<string, string>();
  for (const tj of (tourRows ?? []) as TourJobRow[]) {
    if (tj.tour_kind !== "video") continue;
    const current = videoStatusByProperty.get(tj.property_id);
    if (!current || (VIDEO_RANK[tj.status] ?? 0) > (VIDEO_RANK[current] ?? 0))
      videoStatusByProperty.set(tj.property_id, tj.status);
  }
    ...
    <main className="bg-background text-foreground flex-1 flex flex-col">
      <section className="mx-auto w-full max-w-7xl px-6 lg:px-12 pt-12 pb-20 lg:pt-16 lg:pb-28">
        ...
        {listings.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 lg:gap-8">
            {listings.map((l) => {
              ...
              const tier =
                tierId && tierId in PRICING_TIERS
                  ? PRICING_TIERS[tierId as PricingTierId]
                  : null;
              const isDraft = l.mls_status === "draft";
              const offersCount = offerCountByProperty.get(l.id) ?? 0;
              const stagedCount = stagedCountByProperty.get(l.id) ?? 0;
              const videoStatus = videoStatusByProperty.get(l.id) ?? "none";

              const cc = copy.command;
              const flowHref = `/${lang}/listing/new?id=${l.id}`;
              const photosHref = `/${lang}/listing/new?id=${l.id}&step=5`;
              const publicHref = `/${lang}/property/${l.id}`;
              const paymentDone = payment.tone === "ok";
              const agreementDone = agreement.tone === "ok";

              // Each workstream Lixtara runs for this listing, with live status.
              const stagingWs: Workstream =
                stagedCount > 0
                  ? {
                      icon: "staging",
                      label: cc.wsStaging,
                      value: `${stagedCount} ${cc.stagingDoneUnit}`,
                      tone: "ok",
                    }
                  : {
                      icon: "staging",
                      label: cc.wsStaging,
                      value: cc.stagingNone,
                      tone: "none",
                    };
              const videoWs: Workstream =
                videoStatus === "ready"
                  ? { icon: "video", label: cc.wsVideo, value: cc.videoReady, tone: "ok" }
                  : videoStatus === "failed"
                    ? { icon: "video", label: cc.wsVideo, value: cc.videoFailed, tone: "fail" }
                    : videoStatus === "none"
                      ? { icon: "video", label: cc.wsVideo, value: cc.videoNone, tone: "none" }
                      : {
                          icon: "video",
                          label: cc.wsVideo,
                          value: cc.videoProcessing,
                          tone: "warn",
                        };
              const offersWs: Workstream =
                offersCount > 0
                  ? {
                      icon: "offers",
                      label: cc.wsOffers,
                      value: `${offersCount} ${cc.offersValueUnit}`,
                      tone: "ok",
                    }
                  : {
                      icon: "offers",
              ...
              // Real, derived performance metrics (no fabricated analytics).
              const daysListed = daysSince(l.created_at);
              const aiAssets = stagedCount + (videoStatus === "ready" ? 1 : 0);
              const metrics = [
                { label: cc.metricDays, value: String(daysListed) },
                { label: cc.metricOffers, value: String(offersCount) },
                { label: cc.metricAssets, value: String(aiAssets) },
              ];

              // Recommended next step — first unmet, highest-impact action.
              let nextStep: { text: string; ctaLabel: string; ctaHref: string };
              if (isDraft)
                nextStep = {
                  text: cc.nsComplete,
                  ctaLabel: cc.nsCompleteCta,
                  ctaHref: `${flowHref}&step=6`,
                };
              else if (!paymentDone)
                nextStep = { text: cc.nsPay, ctaLabel: cc.nsPayCta, ctaHref: flowHref };
              else if (!agreementDone)
                nextStep = {
                  text: cc.nsAgreement,
                  ctaLabel: cc.nsAgreementCta,
                  ctaHref: flowHref,
                };
              else if (stagedCount === 0)
                nextStep = {
                  text: cc.nsStaging,
                  ctaLabel: cc.nsStagingCta,
                  ctaHref: photosHref,
                };
              else if (videoStatus === "none" || videoStatus === "failed")
                nextStep = {
                  text: cc.nsVideo,
                  ctaLabel: cc.nsVideoCta,
                  ctaHref: photosHref,
                };
              else if (offersCount > 0)
                nextStep = {
                  text: cc.nsOffers,
                  ctaLabel: cc.nsOffersCta,
                  ctaHref: "#received-offers",
                };
              else
                nextStep = {
                  text: cc.nsOptimized,
                  ctaLabel: cc.nsOptimizedCta,
                  ctaHref: publicHref,
                };
        ...
        {receivedOffers.length > 0 && (
          <div id="received-offers" className="mt-16 lg:mt-24 scroll-mt-24">
